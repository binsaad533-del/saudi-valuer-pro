...
BEGIN
  ...
  ELSIF _user_role = 'financial_manager' THEN
    _role_allowed := (_new_status IN ('first_payment_confirmed', 'final_payment_confirmed'));
  ELSE
    _role_allowed := false;
  END IF;
  ...
  IF _new_status = 'first_payment_confirmed' THEN
    ...
    IF _action_type = 'simulated' THEN
      _payment_ok := true;
    ELSE
      SELECT EXISTS(SELECT 1 FROM public.payments WHERE request_id = _request_id AND payment_stage IN ('first', 'full') AND payment_status = 'paid') INTO _payment_ok;
    END IF;
  ...
  IF _new_status = 'final_payment_confirmed' THEN
    ...
    IF _action_type = 'simulated' THEN
      _payment_ok := true;
    ELSE
      SELECT EXISTS(SELECT 1 FROM public.payments WHERE request_id = _request_id AND payment_stage IN ('final', 'full') AND payment_status = 'paid') INTO _payment_ok;
    END IF;
    IF NOT _payment_ok AND _action_type = 'bypass' THEN
      IF _bypass_justification IS NULL OR length(trim(_bypass_justification)) < 10 THEN
        RETURN jsonb_build_object('success', false, 'error', 'يجب كتابة مبرر واضح (10 أحرف على الأقل) لتجاوز بوابة الدفع');
  ...
  -- APPLY STATUS (cast to enum)
  UPDATE public.valuation_assignments
  SET status = _new_status::assignment_status, updated_at = now()
  WHERE id = _assignment_id;
  ...
  INSERT INTO public.request_audit_log (
    ...
    jsonb_build_object(
      'payment_gate', _is_payment_gate, 'payment_verified', _payment_ok,
      'simulated', (_action_type = 'simulated'), 'bypass_used', (_action_type = 'bypass'),
      'bypass_justification', _bypass_justification, 'user_role', _user_role, 'validated_at', now()::text
    )
  );
...
-- Also fix enforce_post_payment_lock to use correct column name client_name_ar
