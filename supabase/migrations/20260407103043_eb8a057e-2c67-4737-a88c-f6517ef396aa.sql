...
-- ═══════════════════════════════════════════════════════════════
-- FINAL SECURITY LAYER: DB-Driven Workflow Engine
-- ═══════════════════════════════════════════════════════════════

-- 1. Drop existing triggers that we'll recreate
...
-- ═══════════════════════════════════════════════════════════════
-- 2. MASTER FUNCTION: update_request_status (with role validation)
-- ═══════════════════════════════════════════════════════════════
...
DECLARE
  ...
  _user_role text := 'unknown';
  _role_allowed boolean := false;
...
BEGIN
  -- ══ VALIDATE: status must be in the 18-status enum ══
  ...
  -- Set session var so our trigger allows the change
  ...
  -- Auto/system transitions bypass role check
  IF _action_type IN ('auto', 'simulated') THEN
    _role_allowed := true;
  ELSIF _user_role = 'owner' OR _user_role = 'admin' THEN
    -- Owner can do all transitions + bypass
    _role_allowed := true;
  ELSIF _user_role = 'client' THEN
    _role_allowed := (
      (_current_status = 'draft' AND _new_status = 'submitted') OR
      (_current_status = 'scope_generated' AND _new_status = 'scope_approved') OR
      (_current_status = 'client_review' AND _new_status = 'draft_approved')
    );
  ELSIF _user_role = 'inspector' THEN
    _role_allowed := (_current_status = 'inspection_pending' AND _new_status = 'inspection_completed');
  ELSIF _user_role = 'financial_manager' THEN
    _role_allowed := (
      _new_status IN ('first_payment_confirmed', 'final_payment_confirmed')
    );
  ELSE
    _role_allowed := false;
  END IF;
  ...
  IF NOT _role_allowed THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', format('الدور "%s" غير مصرح بالانتقال: %s → %s', _user_role, _current_status, _new_status)
    );
  END IF;
  ...
  -- ══ ISSUED GATE: must come from final_payment_confirmed ══
  ...
  INSERT INTO public.audit_logs (
    ...
    old_data, new_data, description, user_role
  ) VALUES (
    ...
    format('تغيير حالة: %s → %s | دور: %s | نوع: %s%s',
      _current_status, _new_status, _user_role, _action_type,
...
-- ═══════════════════════════════════════════════════════════════
-- 3. TRIGGER: Prevent direct status UPDATE on valuation_assignments
-- ═══════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════
-- 4. TRIGGER: enforce_data_lock on valuation_requests
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- If no assignment linked, allow
  IF _assignment_status IS NULL THEN
    RETURN NEW;
  END IF;

  -- ARCHIVED: block ALL updates
  IF _assignment_status = 'archived' THEN
    RAISE EXCEPTION 'الطلب مؤرشف — لا يمكن إجراء أي تعديل';
  END IF;

  -- ISSUED: block all data updates (allow only updated_at from triggers)
  IF _assignment_status = 'issued' THEN
    RAISE EXCEPTION 'التقرير صادر — لا يمكن تعديل بيانات الطلب';
  END IF;

  -- POST-PAYMENT: block core fields + asset_data
  IF _assignment_status = ANY(_post_payment_statuses) THEN
    IF TG_OP = 'UPDATE' THEN
      IF NEW.valuation_type IS DISTINCT FROM OLD.valuation_type
        ...
        RAISE EXCEPTION 'لا يمكن تعديل بيانات الطلب الأساسية بعد تأكيد الدفع (client_name, asset_data, valuation_type, asset_type, purpose)';
      END IF;
    END IF;
  END IF;
