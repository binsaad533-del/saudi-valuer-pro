  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        ...
        inventory: {
          type: "array",
          description: "جرد كامل لكل الأصول — كل صف في إكسل = أصل منفصل. الصور: قد تكون عدة صور لأصل واحد (زوايا مختلفة، أجزاء داخلية، عدادات) — اجمعها في أصل واحد",
          items: {
            type: "object",
            properties: {
              id: { type: "number" },
...
## ⚠️ قاعدة تجميع الصور (حرجة):
**عدد الملفات ≠ عدد الأصول**
- إذا كانت عدة صور تصوّر **نفس الأصل** من زوايا مختلفة (واجهة أمامية، جانبية، داخلية، عداد/ساعات تشغيل، لوحة بيانات nameplate، تفاصيل قريبة) → **اجمعها في أصل واحد فقط**
- مؤشرات أن الصور لنفس الأصل:
  • نفس اللون والشكل العام
  • نفس الماركة/الشعار المرئي
  • نفس البيئة المحيطة (خلفية، أرضية، موقع)
  • صور قريبة (close-up) لأجزاء من آلة ظاهرة في صورة أخرى
  • صور عداد/ساعات تشغيل/لوحة بيانات تابعة لمعدة مصورة
- اذكر **كل** الصور المصدرية في حقل source مفصولة بفاصلة
- استخرج البيانات من **جميع** الصور مجتمعة (مثلاً: الماركة من صورة، الرقم التسلسلي من صورة أخرى، الحالة من ثالثة)
- أضف في notes عدد الصور المجمّعة: "تم تجميع X صورة لأصل واحد"

## قواعد الجرد الحاسمة:
1. **كل صف في جدول Excel/CSV = أصل منفصل** — لا تدمج الصفوف أبداً
2. **الصور**: حلل كل الصور أولاً، ثم قرر أيها تمثل نفس الأصل وأيها أصول مختلفة — لا تفترض أن كل صورة = أصل مستقل
...
## قواعد إلزامية:
- اذكر مصدر كل أصل بدقة (اسم الملف + الصفحة/الصف/رقم الصورة) — للأصول المجمّعة من عدة صور، اذكر كل أسماء الملفات
...
serve(async (req) => {
  ...
  try {
    ...
    if (hasContent) {
      const imageCount = visionItems.filter(v => v.mimeType.startsWith("image/")).length;
      const clusterHint = imageCount > 1
        ? `\n\n⚠️ تنبيه مهم: تم رفع ${imageCount} صورة. حلل جميع الصور أولاً وحدد هل تمثل أصلاً واحداً من زوايا مختلفة أم أصولاً متعددة. عدد الصور ≠ عدد الأصول بالضرورة. إذا كانت الصور لنفس الآلة/المعدة/العقار → أنشئ أصل واحد فقط واجمع كل البيانات من جميع الصور.`
        : "";
      userContent.push({
        type: "text",
        text: `تم رفع ${fileNames.length} مستند. حلل كل المحتوى واستخرج جرداً كاملاً ودقيقاً لكل الأصول بدون استثناء.\n\nتعليمات مهمة:\n- كل صف في Excel = أصل منفصل\n- الصور: قد تكون عدة صور لأصل واحد — اجمعها ولا تكررها\n- لا تختصر — أعد كل الأصول الحقيقية التي تجدها${clusterHint}`,
      });
