...
serve(async (req) => {
  ...
  try {
    ...
    // Support both new inventory format and old asset format
    const inv = Array.isArray(extractedData.inventory) ? extractedData.inventory : [];
    const firstItem = inv[0];
    const getInvField = (key: string) => firstItem?.fields?.find((f: any) => f.key === key)?.value;
    
    const area = parseFloat(getInvField("area_sqm") || extractedData.asset?.area || "0") || 500;
    const city = getInvField("city") || extractedData.asset?.city || "الرياض";
    const propertyDesc = extractedData.description || extractedData.asset?.description || firstItem?.name || "عقار سكني";
    const purpose = extractedData.suggestedPurpose || "تقييم عام";
    
    // Check for rental/multi-unit from inventory fields
    const allFields = inv.flatMap((item: any) => item.fields || []);
    const hasRental = allFields.some((f: any) => 
      f.key?.includes("rental") || f.label?.includes("إيجار") || f.label?.includes("ريع")
    ) || extractedData.extractedNumbers?.some((n: any) =>
      n.label?.includes("إيجار") || n.label?.includes("ريع")
    );
    const hasMultipleUnits = inv.length > 1 || extractedData.extractedNumbers?.some((n: any) =>
...
- عمر البناء: ${extractedData.building?.age || "—"} سنة`
    ...
    const inventorySummary = inv.length > 0
      ? inv.map((item: any, idx: number) => {
          const fields = (item.fields || []).map((f: any) => `  - ${f.label}: ${f.value} (ثقة ${f.confidence || "—"}%)`).join("\n");
          return `الأصل ${idx + 1}: ${item.name} (${item.type === "real_estate" ? "عقار" : "آلة/معدة"}) — الكمية: ${item.quantity}\n${fields}`;
        }).join("\n\n")
      : "لا يوجد جرد أصول";

    const district = getInvField("district") || extractedData.asset?.district || "غير محدد";
    const deedNumber = getInvField("deed_number") || extractedData.asset?.deedNumber || "غير متوفر";
    const classification = getInvField("classification") || extractedData.asset?.classification || "غير محدد";

    const userPrompt = `بيانات الطلب المستخرجة من المستندات:
━━━ بيانات الأصول ━━━
- وصف الأصل: ${propertyDesc}
...
- التخصص: ${extractedData.discipline || "real_estate"} (${extractedData.discipline_label || "تقييم عقاري"})${buildingInfo}

━━━ جرد الأصول (${inv.length} أصل) ━━━
${inventorySummary}
...
- تعدد أصول: ${hasMultipleUnits ? "نعم" : "لا"}${rentalInfo}
...
قم بتوليد نطاق العمل التفصيلي وتعديلات التسعير المناسبة.`;
        ...
        tools: [
          {
            ...
            function: {
              ...
              parameters: {
                  ...
                  methodologyAnalysis: {
                    ...
                    properties: {
                      primaryApproach: {
                        ...
                        properties: {
                          key: { type: "string" },
                          label: { type: "string", description: "اسم المنهجية بالعربي مثل: أسلوب المقارنة السوقية" },
                          labelEn: { type: "string", description: "اسم المنهجية بالإنجليزي مثل: Sales Comparison Approach" },
                          role: { type: "string", enum: ["primary"], description: "دائماً primary" },
                          confidence: { type: "number" },
                          reason: { type: "string", description: "سبب اختيار هذه المنهجية كرئيسية" },
                          ivsReference: { type: "string" },
                        },
                        required: ["key", "label", "labelEn", "role", "confidence", "reason"],
                      },
                      secondaryApproach: {
                        type: "object",
                        properties: {
                          ...
                          label: { type: "string" },
                          labelEn: { type: "string" },
                          role: { type: "string", enum: ["secondary"] },
                          confidence: { type: "number" },
                          reason: { type: "string" },
                          ivsReference: { type: "string" },
                        },
                      ...
                      allApproaches: {
                        ...
                        items: {
                          ...
                          properties: {
                            ...
                            label: { type: "string" },
                            labelEn: { type: "string" },
                            role: { type: "string", enum: ["primary", "secondary", "supporting"] },
                            confidence: { type: "number" },
                            reason: { type: "string" },
                            ivsReference: { type: "string" },
                          },
                        ...
                        description: "جميع المنهجيات الثلاث (مقارنة سوقية، دخل، تكلفة) مع تحديد دور كل منها"
                      },
                      justification: { type: "string", description: "تبرير شامل لاختيار المنهجيات وفق IVS 105" },
                    },
                    required: ["primaryApproach", "secondaryApproach", "allApproaches", "justification"],
                  },
                  scope: {
                    type: "object",
                    properties: {
                ...
                required: ["disciplineAnalysis", "purposeAnalysis", "basisOfValueAnalysis", "methodologyAnalysis", "scope", "pricingAdjustments"],
    ...
    const result = {
      scope: {
        ...
        basisOfValueAnalysis: aiData.basisOfValueAnalysis || {
          selectedBasis: "القيمة السوقية",
          selectedBasisEn: "Market Value",
          confidence: 70,
          reason: "أساس القيمة الافتراضي بناءً على غرض التقييم",
          ivsReference: "IVS 104.20",
          allBases: [{ key: "market_value", label: "القيمة السوقية", labelEn: "Market Value", confidence: 70, reason: "افتراضي" }],
        },
        methodologyAnalysis: aiData.methodologyAnalysis || {
          primaryApproach: { key: "market", label: "أسلوب المقارنة السوقية", labelEn: "Sales Comparison Approach", role: "primary", confidence: 70, reason: "المنهجية الافتراضية" },
          secondaryApproach: { key: "cost", label: "أسلوب التكلفة", labelEn: "Cost Approach", role: "secondary", confidence: 50, reason: "منهجية داعمة" },
          allApproaches: [
            { key: "market", label: "أسلوب المقارنة السوقية", labelEn: "Sales Comparison Approach", role: "primary", confidence: 70, reason: "افتراضي" },
            { key: "income", label: "أسلوب الدخل", labelEn: "Income Approach", role: "supporting", confidence: 40, reason: "لم يتوفر بيانات إيجارية" },
            { key: "cost", label: "أسلوب التكلفة", labelEn: "Cost Approach", role: "secondary", confidence: 50, reason: "منهجية داعمة" },
          ],
          justification: "تم اختيار المنهجيات بناءً على طبيعة العقار والبيانات المتاحة",
        },
      },
      pricing: {
        basePrice,
